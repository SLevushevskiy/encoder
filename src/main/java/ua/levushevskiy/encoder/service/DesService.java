...
public class DesService {
    ...
    public DesResponse encode(String keyRequest, String messageRequest) {
        StringBuilder keyNew = new StringBuilder(keyRequest);
        while (keyNew.length() < 16)
            keyNew.insert(0, "0");
        DesResponse desResponse = new DesResponse();
        byte[] message = parseBytes(messageRequest);
        byte[] key = parseBytes(keyNew.toString());
        byte[] ciphertext = new byte[message.length];
        for (int i = 0; i < message.length; i += 8) {
            long subkeys[] = startChange(key);
            ...
            for (int k = 0; k < 16; k++) {
                int previous_l = left;
                left = right;
                right = previous_l ^ feistel(right, subkeys[k]);
                desResponse.getRounds().put("Cipher #" + (k + 1), printRaund(right, left));
            }
        ...
        desResponse.setCipher(Cipher.toString());
        desResponse.setKey(Key.toString());
        desResponse.setMessage(Message.toString());
        return desResponse;
    }

    private long[] startChange(byte[] key) {
    ...
    private String printRaund(int right, int left) {
        StringBuilder Cipher = new StringBuilder();
        byte[] ciphertext;
        long rl = (right & 0xFFFFFFFFL) << 32 | (left & 0xFFFFFFFFL);
        long fp = permute(FP, 64, rl);
